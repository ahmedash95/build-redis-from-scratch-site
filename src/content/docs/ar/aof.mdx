...
الجزء ده احنا هنطبق فيه علي الـ [data persistence](https://redis.io/docs/management/persistence/) ودي فيتشر مهمة اي قاعدة بيانات بتحتاجها حتي لو InMemory لان فيه حالات كتير
...
- بنعمل go routine علشان طول ما السيرفر شغال. هتسجل الـ AOF file كل ١ ثانيه علي الـ disk.

<Alert type="info">
 فكره اننا نعمل sync كل ثانيه ده بيضمن
...
كل اللي احنا محتاجين نعمله اننا نستخدم ال NewAof في main.go و مع كل ريكوست من الـ client نعمل write للـ AOF file
...
<Alert type="info">
لاحظ اننا بنكتب الcommands اللي بتعمل set بس .. لان اي اوامر تانيه زي get, hget, و HGETALL مش هتحتاج نخزنها
