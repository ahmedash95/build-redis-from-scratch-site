...
<Alert type="info">
لاحظ اننا كتبنا الاسم capital لان الاوامر دي في redis بتبقي [case insensitive](https://redis.io/commands/command/#:~:text=Redis%20command%20names%20are%20case%2Dinsensitive.).
</Alert>
...
الـ [Set](https://redis.io/commands/set/) في redis عباره عن key value. في اي وقت هتعمل SET لـ key بـ value معينه. و ف اي وقت تقدر تجيبها تاني بالـ GET command.

ف الـ Datastructure اللي هنستخدمها هي [Hash Map](https://yourbasic.org/golang/maps-explained/) او `map[string]string`
...
<Alert type="info">
بنستخدم الـ [sync.RWMutex](https://pkg.go.dev/sync#RWMutex) لان المفروض ان الـ Server بتاعنا هيستقبل الـ requests بشكل concurrent. و بنستخدم الـ RWMutex علشان نتاكد ان الـ SETs map مش هتتعدل من اكثر من thread في نفس الوقت.
</Alert>
...
كدا احنا عملنا الـ HSET & HGET commands. فيه كوماند زياده المره دي عن الـSETS اسمه [HGETALL](https://redis.io/commands/hgetall/). وده بيرجع كل الـ keys و الـ values في الـ hash
